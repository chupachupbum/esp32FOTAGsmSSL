...
static int has_valid_chars(const char *str, const char *matrix) {
  ...
  for (i = 0; i < len; i++)
    if (contains(str[i], matrix, mlen) == 0) return 0;
...
static int compare_prerelease(char *x, char *y) {
  ...
  while (1) {
    if ((xptr = strchr(lastx, DELIMITER[0])) == NULL) xptr = x + xlen;
    if ((yptr = strchr(lasty, DELIMITER[0])) == NULL) yptr = y + ylen;
...
int semver_satisfies(semver_t x, semver_t y, const char *op) {
  ...
  /* Caret operator */
  if (first == SYMBOL_CF) return semver_satisfies_caret(x, y);

  /* Tilde operator */
  if (first == SYMBOL_TF) return semver_satisfies_patch(x, y);

  /* Strict equality */
  if (first == SYMBOL_EQ) return semver_eq(x, y);
...
static int char_to_int(const char *str) {
  ...
  for (i = 0; i < len; i++)
    if (contains(str[i], VALID_CHARS, mlen)) buf += (int)str[i];
