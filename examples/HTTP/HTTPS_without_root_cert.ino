/**
   esp32 firmware OTA

   Purpose: Perform an OTA update from a bin located on a webserver (HTTPS) without having a root cert

   Setup:
   Step 1 : Set your WiFi (ssid & password)
   Step 2 : set esp32FotaGsmSSL()

   Upload:
   Step 1 : Menu > Sketch > Export Compiled Library. The bin file will be saved in the sketch folder (Menu > Sketch > Show Sketch folder)
   Step 2 : Upload it to your webserver
   Step 3 : Update your firmware JSON file ( see firwmareupdate )

*/

#include <Arduino.h>
#include <FS.h>
#include <SPIFFS.h>
#include <WiFi.h>
#include <esp32FotaGsmSSL.h>

// Change to your WiFi credentials
const char *ssid     = "";
const char *password = "";

// esp32FotaGsmSSL esp32FotaGsmSSL("<Type of Firmware for this device>", <this version>, <validate signature>, <allow insecure https>);
esp32FotaGsmSSL esp32FotaGsmSSL("esp32-fota-http", 1, false, true);

void setup_wifi() {
  delay(10);
  Serial.print("Connecting to ");
  Serial.println(ssid);

  WiFi.begin(ssid, password);

  while (WiFi.status() != WL_CONNECTED) {
    delay(500);
    Serial.print(".");
  }

  Serial.println("");
  Serial.println(WiFi.localIP());
}

void setup() {
  esp32FotaGsmSSL.checkURL = "https://server/fota/fota.json";
  Serial.begin(115200);
  setup_wifi();
}

void loop() {
  bool updatedNeeded = esp32FotaGsmSSL.execHTTPcheck();
  if (updatedNeeded) {
    esp32FotaGsmSSL.execOTA();
  }

  delay(2000);
}
